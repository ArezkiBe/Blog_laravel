    ...
    <section class="flex-1 w-full max-w-4xl mx-auto py-12 flex flex-col gap-8">
        <article class="px-4 py-3 bg-white border shadow rounded-xl space-y-4">
            ...
            <div class="flex items-center space-x-4">
                ...
                <!-- Comment Icon -->
                <a href="#replies" class="flex items-center text-gray-500 hover:text-gray-700 focus:outline-none">
                    <!-- Icone de message -->
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <!-- Formulaire de réponse au post -->
        <form action="{{ route('replies.store', ['post' => $post]) }}" method="post" class="px-4 py-3 bg-white border shadow rounded-xl space-y-2">
            @csrf
            <div>
        ...
        <section id="replies" class="flex-1 w-full max-w-4xl mx-auto py-12 flex flex-col gap-8">
            @if($post->replies->whereNull('parent_id')->isNotEmpty())
